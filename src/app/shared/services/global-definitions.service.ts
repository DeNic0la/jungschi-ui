...
export class GlobalDefinitionsService {
  ...
  private readonly foodIntolerances$ = this.http
    .get<GlobalDefinitionDto[]>(`${this.apiUrl}/food-intolerances`)
    .pipe(retry({ count: 3, delay: 6_000 }), shareReplay({ bufferSize: 1, refCount: false }));
  public readonly globalFoodIntolerances$ = this.foodIntolerances$.pipe(startWith([]));
  ...
  private readonly allergies$ = this.http
    .get<GlobalDefinitionDto[]>(`${this.apiUrl}/allergies`)
    .pipe(retry({ count: 3, delay: 6_000 }), shareReplay({ bufferSize: 1, refCount: false }));
  public readonly globalAllergies$ = this.allergies$.pipe(startWith([]));
