...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, Button, InputText, Panel, Message, TranslatePipe],
  template: `
    <div class="page-container max-w-3xl">
      <p-panel [header]="'features.profile.title' | translate">
        @if (loading()) {
          <div class="flex justify-center items-center p-8">
            <p-message severity="info" [text]="'features.profile.status.loading' | translate" />
          </div>
        } @else if (error()) {
          <div class="flex justify-center items-center p-8">
          ...
          >
            <div class="flex flex-col gap-2">
              <label for="username" class="font-semibold text-surface-900 dark:text-surface-0">
                {{ 'features.profile.form.username.label' | translate }}
              </label>
            ...
            <div class="flex flex-col gap-2">
              ...
              <p class="text-sm text-surface-600 dark:text-surface-400">
                {{ 'features.profile.form.phoneNumber.help' | translate }}
              </p>
              <input
                pInputText
                id="phoneNumber"
            ...
            <div class="flex flex-wrap gap-4 mt-4">
              ...
              />
              <p-button
                [label]="'features.profile.actions.changePassword' | translate"
                type="button"
                severity="secondary"
                icon="pi pi-key"
              ...
              />
              <p-button
                [label]="'features.profile.actions.manageAccount' | translate"
                type="button"
                severity="info"
...
export class ProfileComponent implements OnInit {
  ...
  protected async saveProfile(): Promise<void> {
    ...
    try {
      ...
      this.error.set(this.t('common.status.actionFailed'));
    } finally {
  ...
  protected async changePassword(): Promise<void> {
    try {
      ...
      this.error.set(this.t('features.profile.status.passwordChangeFailed'));
    }
  }
  ...
  protected async manageAccount(): Promise<void> {
    try {
      ...
      this.error.set(this.t('features.profile.status.accountManagementFailed'));
    }
  }
